			...
			<div class="inner">
				...
				<section>
					<header class="main">
						...
						<input type="button" value="新增" onclick="location.href='<c:url value='/_05_teacStu.insertdataStudMain.controller' />'">
					</header>
						<form method="post" action="<c:url value='/_05_teacStu.searchAllLikeStud.controller' />">
							<input type="text" name="search" id="search" placeholder="Search" style="width:80%" />
						</form>
						<br>
						<div>
						<form method="post" action="<c:url value='/_05_teacStu.switchsequenceStud.controller' />">
						<h3>請選擇排列方式</h3>
						...
						<div>
						<form method="post" action="<c:url value='/_05_teacStu.searchAllStudByPrice.controller' />">
									...
									<tbody>
										<tr>
											<td>${bean.studno}</td>
     										...
     										<td>${bean.learnloc}</td>
     										<td><img height='100' width='80'src="<c:url value='/_05_teacStu.picsaveStud.controller?id=${bean.studno}' />"></td>
     										<td><form method="post"
												action="<c:url value='/_05_teacStu.updatedataStudMain.controller' />">
												<button name="studno" value="${bean.studno}">
													修改
												</button>
											</form></td>
											<td><button class="delete" name="studno"
												value="${bean.studno}">刪除</button></td>
										</tr>
									</tbody>
									</c:forEach>
									</table>
...
//                   cancelButtonText: '取消',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                          //專案名稱+servlet
                          url:'/SpringBoot_Team5/_05_teacStu.deletedataStud.controller',
                          method:"get",
                          dataType:"text",
