...
<body data-spy="scroll" data-target=".onpage-navigation"
	...
	<main>
		...
		<!-- 				<button style="float: left" class="btn btn-b btn-circle" -->
						...
						<div class="post-section">
							...
							<div class="post-footer">
								...
								<div class="reply-list" id="fuckyoureplylist">
									...
									<input type="hidden" value="${member.memNickName}"
										id="memberNickName" class="memberNickName"> <input
										type="hidden" value="${member.memberID}" name="memberid"
										id="memberIDforAjax"> <input type="hidden"
										value="${member.account}" name="accountAjax" id="accountAjax">

									<c:forEach var="respBean" items="${bean.responseHala}">
										<div class="reply-item">
											<div class="reply-avatar">
												<img id="headPhoto" class="headPhoto"
													src="<c:url value='/_01_member.ShowPhoto.controller?account=${respBean.memberBean.account}'/>"
													alt="">
											</div>
							...
							<div class="post-footer">
								...
								<div class="reply-list" id="fuckyoureplylist123">
									...
									<c:forEach var="respAnswerBean"
										...
										<input type="hidden" value="${member.memNickName}"
											id="memberNickName" class="memberNickName">

										<input type="hidden" value="${member.account}"
											name="accountAjax" id="accountAjax">

										<div class="reply-item">
											<div class="reply-avatar">

												<img id="headPhoto" class="headPhoto"
													src="<c:url value='/_01_member.ShowPhoto.controller?account=${respAnswerBean.memberBean.account}'/>"
													alt="">
				...
				<div class="comment-form" style='margin-left: 130px; margin-top: 0'>
					...
					<form
						...
						<button class="btn btn-round btn-d" type="submit"
							onclick="submitForm()">Post comment</button>
					</form>
				</div>
			</div>
			<!-- 新增回覆區域結束 -->
	...
	<script>
		function checkLogin() {
			// 檢查使用者是否已登入，如果未登入則顯示警告訊息
			if (!<c:out value="${not empty member}"/>) {
				alert('請先登入才能回復喔！');
				// 將使用者導向登入頁面
	...
	<script>
		function handleKeyPress(event) {
			...
			if (event.key == 'Enter') {
				...
				console.log("確認到Enter")
				var newTextValue = document.getElementById("textForRespHala").value;
				console.log("newTextValue=" + newTextValue)
				var mID = document.getElementById("memberIDforAjax").value;
				console.log("mID=" + memberIDforAjax)
				// 				var headPhoto = document.getElementById("headPhoto").value;
				// 				console.log("headPhoto="+headPhoto)
				var acc = document.getElementById("accountAjax").value;
				console.log("acc=" + accountAjax)
				...
				img1.setAttribute("src",
						"<c:url value='/_01_member.ShowPhoto.controller?account="
								+ acc + "'/>")
	...
	<script>
		function handleKeyPress2(event) {
			...
			if (event.key == 'Enter') {
				...
				var newTextValue = document
						.querySelectorAll('.textForRespAnswer');
				...
				for (var i = 0; i < newTextValue.length; i++) {
					...
					console.log("newTextValue[i]="
							+ newTextValue[i].value.length);
					if (newTextValue[i].value.length != 0) {
						...
						var list = document
								.querySelectorAll('#fuckyoureplylist123');
						...
						if (!list) {
							console
									.error("Cannot find element with id 'reply-List'");
							return;
						}
